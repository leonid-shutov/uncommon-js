...
const loadDir = async (parentContext, parentContainer, dirPath, key) => {
  ...
  // `self` is the module itself, writes through `self` land on the module
  ...
  for (let { name } of directories) {
    ...
    // Grouping dirs are transparent, subdirectories load into the group's parent
    else if (dirType === DirTypes.Group) await loadDir(parentContext, parentContainer, location, name);
