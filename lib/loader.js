...
const orderFiles = (files) => {
  const hasAfter = files.includes('after.js');
  const hasBefore = files.includes('before.js');
  const ordered = files.filter((file) => chronologicalFiles.includes(file));
  if (hasAfter) ordered.unshift('after.js');
  if (hasBefore) ordered.push('before.js');
  return ordered;
};
