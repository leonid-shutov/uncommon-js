...
const { readdirSorted, safeAssign } = require('./util.js');
...
const loadFile = async (context, container, filePath, key) => {
  ...
  const sandbox = { ...context, $: container, self: container };
...
const loadDir = async (context, container, dirPath, key) => {
  ...
  const files = [];
  const directories = [];
  const entries = await readdirSorted(dirPath, { withFileTypes: true });
