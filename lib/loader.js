...
const sortDirectories = (directories) => {
  const commonDir = directories.find(({ name }) => name === 'common');
  if (commonDir === undefined) return directories;
  const ordered = directories.filter(({ name }) => name !== 'common');
  ordered.unshift(commonDir);
  return ordered;
};
...
const loadDir = async (context, container, dirPath, key) => {
  ...
  for (let { name } of sortDirectories(directories)) {
    const location = path.join(dirPath, name);
